...
class Lib_upload {
     ...
     * @return an array with upload result
     */
    private function upload_file($file_name, $tmp_name) {
        ...
        if ( $result['isSuccessful'] = move_uploaded_file($tmp_name, $target_path) ) {
            $result['message'] = $target_path;
        } else {
            $result['message'] = '发生未知错误.<br />';
        }
        return $result;
    }

    /**
     * Generate random file name for uploaded files.
